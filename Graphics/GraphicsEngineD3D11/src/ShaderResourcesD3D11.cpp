    ...
    m_StaticSamplers(STD_ALLOCATOR_RAW_MEM(StaticSamplerAttribs, GetRawAllocator(), "Allocator for vector<StaticSamplerAttribs>"))
{
    ...
    public:
        ...
        void OnNewCB(const D3DShaderResourceAttribs& CBAttribs)
        {
            ...
            Resources.m_MaxCBBindPoint = stl::max(Resources.m_MaxCBBindPoint, static_cast<MaxBindPointType>(CBAttribs.BindPoint + CBAttribs.BindCount-1));
        }
        ...
        void OnNewTexUAV (const D3DShaderResourceAttribs& TexUAV)
        {
            ...
            Resources.m_MaxUAVBindPoint = stl::max(Resources.m_MaxUAVBindPoint, static_cast<MaxBindPointType>(TexUAV.BindPoint + TexUAV.BindCount-1));
        }
        ...
        void OnNewBuffUAV(const D3DShaderResourceAttribs& BuffUAV)
        {
            ...
            Resources.m_MaxUAVBindPoint = stl::max(Resources.m_MaxUAVBindPoint, static_cast<MaxBindPointType>(BuffUAV.BindPoint + BuffUAV.BindCount-1));
        }
        ...
        void OnNewBuffSRV(const D3DShaderResourceAttribs& BuffSRV)
        {
            ...
            Resources.m_MaxSRVBindPoint = stl::max(Resources.m_MaxSRVBindPoint, static_cast<MaxBindPointType>(BuffSRV.BindPoint + BuffSRV.BindCount-1));
        }
        ...
        void OnNewSampler(const D3DShaderResourceAttribs& SamplerAttribs)
        {
            ...
            Resources.m_MaxSamplerBindPoint = stl::max(Resources.m_MaxSamplerBindPoint, static_cast<MaxBindPointType>(SamplerAttribs.BindPoint + SamplerAttribs.BindCount-1));
            ...
            if (SamplerAttribs.IsStaticSampler())
            {
                ...
                for (; ssd < ShdrDesc.NumStaticSamplers; ++ssd)
                {
                    ...
                    if (StreqSuff(SamplerAttribs.Name, StaticSamplerDesc.SamplerOrTextureName, CombinedSamplerSuffix))
                    {
                        ...
                        m_StaticSamplers.emplace_back(SamplerAttribs, stl::move(pSampler));
                        break;
                    }
                }
        ...
        void OnNewTexSRV(const D3DShaderResourceAttribs& TexAttribs)
        {
            ...
            Resources.m_MaxSRVBindPoint = stl::max(Resources.m_MaxSRVBindPoint, static_cast<MaxBindPointType>(TexAttribs.BindPoint + TexAttribs.BindCount-1));
        }
...
void ShaderResourcesD3D11::InitStaticSamplers(StaticSamplerVector&& StaticSamplers)
{
    ...
    for(auto& Sam : StaticSamplers)
        m_StaticSamplers.emplace_back(stl::move(Sam));
}
...
void ShaderResourcesD3D11::SetStaticSamplers(ShaderResourceCacheD3D11& ResourceCache)const
{
    ...
    for (const auto& SS : m_StaticSamplers)
    {
        ...
        auto EndBindPoint = stl::min( static_cast<Uint32>(SamAttribs.BindPoint) + SamAttribs.BindCount, NumCachedSamplers);
