...
namespace Diligent
{
    void GenerateMipsVkHelper::GetGlImageFormat(const TextureFormatAttribs& FmtAttribs, stl::array<char, 16>& GlFmt)
    {
        size_t pos = 0;
        GlFmt[pos++] = 'r';
    ...
    stl::array<RefCntAutoPtr<IPipelineState>, 4> GenerateMipsVkHelper::CreatePSOs(TEXTURE_FORMAT Fmt)
    {
        ShaderCreationAttribs CSCreateAttribs;
        stl::array<RefCntAutoPtr<IPipelineState>, 4> PSOs;
    ...
    stl::array<RefCntAutoPtr<IPipelineState>, 4>& GenerateMipsVkHelper::FindPSOs(TEXTURE_FORMAT Fmt)
    {
    ...
    void GenerateMipsVkHelper::GenerateMips(TextureViewVkImpl& TexView, DeviceContextVkImpl& Ctx, IShaderResourceBinding& SRB)
    {
        ...
        for (uint32_t TopMip = 0; TopMip < TexDesc.MipLevels - 1; )
        {
            ...
            stl::array<IDeviceObject*, MaxMipsHandledByCS> MipLevelUAVs;
            for (Uint32 u = 0; u < MaxMipsHandledByCS; ++u)
                MipLevelUAVs[u] = pTexVk->GetMipLevelUAV(stl::min(TopMip + u + 1, TexDesc.MipLevels - 1));
            pOutMipVar->SetArray(MipLevelUAVs.data(), 0, MaxMipsHandledByCS);

            SubresRange.baseMipLevel = TopMip + 1;
            SubresRange.levelCount = stl::min(4u, TexDesc.MipLevels - (TopMip + 1));
