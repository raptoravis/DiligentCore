...
#define LOG_BUFFER_ERROR_AND_THROW(...) LOG_ERROR_AND_THROW("Buffer \"", BuffDesc.Name ? BuffDesc.Name : "", "\": ", ##__VA_ARGS__);
    ...
    m_DynamicOffsetAlignment = stl::max(Uint32{4}, static_cast<Uint32>(DeviceLimits.optimalBufferCopyOffsetAlignment));
    ...
    if (m_Desc.BindFlags & BIND_UNORDERED_ACCESS)
    {
        ...
        m_DynamicOffsetAlignment = stl::max(m_DynamicOffsetAlignment, static_cast<Uint32>(DeviceLimits.minTexelBufferOffsetAlignment));
        m_DynamicOffsetAlignment = stl::max(m_DynamicOffsetAlignment, static_cast<Uint32>(DeviceLimits.minStorageBufferOffsetAlignment));
    }
    ...
    if (m_Desc.BindFlags & BIND_UNIFORM_BUFFER)
    {
        ...
        m_DynamicOffsetAlignment = stl::max(m_DynamicOffsetAlignment, static_cast<Uint32>(DeviceLimits.minUniformBufferOffsetAlignment));
    }
...
BufferVkImpl :: ~BufferVkImpl()
{
    ...
    if(m_VulkanBuffer != VK_NULL_HANDLE)
        m_pDevice->SafeReleaseDeviceObject(stl::move(m_VulkanBuffer), m_Desc.CommandQueueMask);
    ...
    try
    {
        ...
        if( ViewDesc.ViewType == BUFFER_VIEW_UNORDERED_ACCESS || ViewDesc.ViewType == BUFFER_VIEW_SHADER_RESOURCE )
        {
            ...
            *ppView = NEW_RC_OBJ(BuffViewAllocator, "BufferViewVkImpl instance", BufferViewVkImpl, bIsDefaultView ? this : nullptr)
                                (GetDevice(), ViewDesc, this, stl::move(View), bIsDefaultView );
        }
