    ...
    try
    {
        ...
        else if( ViewDesc.ViewType == TEXTURE_VIEW_UNORDERED_ACCESS )
        {
            VERIFY( ViewDesc.NumArraySlices == 1 || 
                    m_Desc.Type == RESOURCE_DIM_TEX_3D && ViewDesc.NumDepthSlices == stl::max(m_Desc.Depth >> ViewDesc.MostDetailedMip, 1U) ||
                    ViewDesc.NumArraySlices == m_Desc.ArraySize,
                               ...
                               Uint32 DstZ)
{
    ...
    if( pSrcBox == nullptr )
    {
        SrcBox.MaxX = stl::max( SrcTexDesc.Width >> SrcMipLevel, 1u );
        if( SrcTexDesc.Type == RESOURCE_DIM_TEX_1D || 
            SrcTexDesc.Type == RESOURCE_DIM_TEX_1D_ARRAY )
            SrcBox.MaxY = 1;
        else
            SrcBox.MaxY = stl::max( SrcTexDesc.Height >> SrcMipLevel, 1u );

        if( SrcTexDesc.Type == RESOURCE_DIM_TEX_3D )
            SrcBox.MaxZ = stl::max( SrcTexDesc.Depth >> SrcMipLevel, 1u );
        else
            SrcBox.MaxZ = 1;
        pSrcBox = &SrcBox;
    }
